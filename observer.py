...
class DeviceObserver:
    def __init__(self, host: str, publisher: 'MQTTPublisher'):
        self.host = host
    ...
    async def observe(self):
        ...
        watchdog_task = asyncio.create_task(self._start_watchdog())
        while self.running:
            while not self.client:
                await self._connect()
            try:
                async for status in self.client.observe_status():
                    await self.signal_state(status)
            except ValueError:
    ...
    async def signal_state(self, status):
        self.last_update = time.monotonic()
        await self.signal_online()
        await self.publisher.publish_state(self.host, status)
...
class MultipleDeviceObserver:
    def __init__(self, hosts: list[str], publisher: 'MQTTPublisher'):
        self.clients = [DeviceObserver(host, publisher) for host in hosts]
        self.publisher = publisher

    async def observe_all(self) -> None:
        logger.info("Started to observe status")
        await MultipleDeviceObserver.execute_many(lambda client: client.observe(), self.clients)

    async def shutdown(self):
        await MultipleDeviceObserver.execute_many(lambda client: client.stop(), self.clients)

    @staticmethod
    async def execute_many(f: Callable[[T], Coroutine], cont: list[T], timeout: int = 0):
    ...
    @staticmethod
    @contextmanager
    def create(hosts, publisher):
        client = MultipleDeviceObserver(hosts, publisher)
        yield client
        client.shutdown()
